...
class RemoteATCommandPacket(XBeePacket):
    def __init__(self, frame_id, x64bit_addr, x16bit_addr, tx_options, command, parameter=None):
        if frame_id > 255 or frame_id < 0:
            frame_id = 0
        super().__init__(0x17)
        self.frame_id = frame_id
...
class RemoteATCommandResponsePacket(XBeePacket):
    ...
    def create_packet(raw):
        ...
        if len(raw) > 19:
            resp = raw[18:-1]
        return RemoteATCommandResponsePacket(raw[4], raw[5:13], raw[13:15], raw[15:17], raw[17], resp)
