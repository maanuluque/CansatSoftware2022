...
def receive_payload_package():
    ...
    if command != 0:
        # MAKE TRY CATCH ON ALL XBEE FUNCTIONS
...
while(True):
    # Just for seeing if cansat crashes
    if (bool_led == True):
        pin_led.high()
        bool_led = False
    else:
    ...
    if simulation_mode == 'True' and sim_activated == 'True':
        if sim_pressure == None:
            ...
            if time.ticks_ms() - new_sim_time > 125:
                ...
                print("Getting: {}".format(plist[index]))
                if (index < lenplist - 1):
                    index += 1
                new_sim_time = time.ticks_ms()
    else:
    ...
    if altitude > APOGEE_ALTITUDE and hasReachApogee == "False": # Make LAUNCH state
        hasReachApogee = "True"
    ...
    if current_state == "PRE-DEPLOY":
        ...
        if entry == True:
            electro.start_motor(2**14) # CHANGE THIS FOR THREADED PID
            entry = False
